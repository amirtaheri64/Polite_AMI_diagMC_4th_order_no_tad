...
def Integrand(M,NUMBER):   # For now number=1
  ...
  ##################################
  #Begin
  if M==1 or M==0:   # First order term is our Normalization
    out.append(1.0)
    out.append(1.0)
    return 1.0
  #if M==0 or M==3:
  ...
  ###################################
  '''
  s_list=deepcopy(S_list[M-2][NUMBER-1])
  p_list_freq=deepcopy(P_list_freq[M-2][NUMBER-1])
  p_list_mnta=deepcopy(P_list_mnta[M-2][NUMBER-1])
  r_freq=deepcopy(R_list_freq[M-2][NUMBER-1])
  r_mnta=deepcopy(R_list_mnta[M-2][NUMBER-1])
  g_sym=deepcopy(G_sym_list[M-2][NUMBER-1])
  F=deepcopy(F_list[M-2][NUMBER-1])
  ...
  out.append((-1)**(M+F)*(U**M)*multiplicity(M,NUMBER)*G_val.real/4**M/pi**(2*M))
  out.append((-1)**(M+F)*(U**M)*multiplicity(M,NUMBER)*G_val.imag/4**M/pi**(2*M))
...
def acp_rt(M_P,M_O,NUMBER_P,NUMBER_O,int_P,int_O):  # Return the acceptance ratio
  ...
  if M_P>=(highest_order+1) or M_P<=-1:  # If the order is out of range
    val=0.0
    return val,sign_O,0
...
def new_state(int_O,M_O,NUMBER_O):  # Accept or reject the proposed state
  ...
  if M_P<=-1 or M_P>=(highest_order+1):
...
for m in range (2,4):
  ...
  S_list[m-2][number-1]=deepcopy(s_arr)

  file_name_P_freq = 'P_freq_m_'+str(m)+'_num_'+str(number)
  with open(file_name_P_freq,"rb") as fp:
    p_arr_freq=pickle.load(fp)
  #print 'P_list_freq = ', p_list_freq
  P_list_freq[m-2][number-1]=deepcopy(p_arr_freq)

  file_name_P_mnta = 'P_mnta_m_'+str(m)+'_num_'+str(number)
  with open(file_name_P_mnta,"rb") as fp:
    p_arr_mnta=pickle.load(fp)
  #print 'P_list_mnta = ', p_list_mnta
  P_list_mnta[m-2][number-1]=deepcopy(p_arr_mnta)

  file_name_R_freq = 'R_freq_m_'+str(m)+'_num_'+str(number)
  with open(file_name_R_freq,"rb") as fp:
    r_arr_freq=pickle.load(fp)
  #print 'R_freq = ', r_freq
  R_list_freq[m-2][number-1]=deepcopy(r_arr_freq)

  file_name_R_mnta = 'R_mnta_m_'+str(m)+'_num_'+str(number)
  with open(file_name_R_mnta,"rb") as fp:
    r_arr_mnta=pickle.load(fp)
  #print 'R_mnta = ', r_mnta
  R_list_mnta[m-2][number-1]=deepcopy(r_arr_mnta)


  file_name_G_sym = 'G_sym_m_'+str(m)+'_num_'+str(number)
  with open(file_name_G_sym,"rb") as fp:
    g_arr_sym=pickle.load(fp)
  G_sym_list[m-2][number-1]=deepcopy(g_arr_sym)
  #print 'G_sym = ', g_sym
...
output = measurement(chains[0],chains[1],chains[2], chains[3])  # Output
